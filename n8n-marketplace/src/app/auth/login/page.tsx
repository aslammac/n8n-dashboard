...
export default function LoginPage() {
  ...
  const [newsletter] = React.useState(true);
  ...
  const handleGoogleLogin = () => {
    window.location.href = `${apiBase}/auth/google?newsletter=${newsletter}`;
  };
  ...
  const showEmailLogin = process.env.NEXT_PUBLIC_ENABLE_EMAIL_LOGIN === "true";

  return (
    <AuthShell
      title="Sign in"
      subtitle="Access thousands of FlowStore workflows."
      footer={
        <>
    ...
    >
      <div className="space-y-3">
        <button
          ...
          className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-border bg-surface hover:bg-surface-2 text-sm font-medium transition-colors"
        >
          <GoogleIcon />
      ...
      {showEmailLogin && (
        <>
          <div className="relative my-6 text-center">
            <span className="absolute inset-0 top-1/2 border-t border-border" />
            <span className="relative bg-card px-3 text-xs text-fg-subtle">
